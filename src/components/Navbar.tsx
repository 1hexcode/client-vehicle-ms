  ...
  return (
    <nav className="glass fixed top-6 left-1/2 -translate-x-1/2 w-[90%] max-w-6xl z-[1000] px-8 py-3 flex justify-between items-center">
      ...
      <div className="hidden md:flex gap-8 items-center">
        ...
        {user?.role === 'Admin' && <Link href="/admin/dashboard" className="hover:text-primary transition-colors">Admin Panel</Link>}
        {user?.role === 'Staff' && <Link href="/staff/dashboard" className="hover:text-primary transition-colors">Staff Panel</Link>}
        {user?.role === 'Customer' && <Link href="/customer/dashboard" className="hover:text-primary transition-colors">My Profile</Link>}
